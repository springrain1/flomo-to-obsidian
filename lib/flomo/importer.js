...
class FlomoImporter {
    ...
    async import() {
        ...
        const flomo = new core_1.FlomoCore(dataExport, syncedMemoIds, flomoTarget, filterTags);
    ...
    async importFlomoFile(filePath, mergeDayFile = true) {
        ...
        // 获取标签过滤配置
        const filterTags = this.config.filterByTags ? (this.config.syncTags || []) : [];
        // 将已同步ID、flomoTarget和标签过滤传递给FlomoCore
        const flomo = new core_1.FlomoCore(flomoData, syncedMemoIds, flomoTarget, filterTags);
