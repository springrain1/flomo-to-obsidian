...
class FlomoImporter {
    ...
    async importMemos(flomo) {
        ...
        const memoTarget = this.config["memoTarget"] || 'memos';
        const mergeByDate = thinoCompatible ? true : (this.config["mergeByDate"] || false);
        // Thino 模式：使用自定义前缀（可为空）；原始模式：使用 memo@ 前缀
        const filenamePrefix = thinoCompatible
            ? (this.config["filenamePrefix"] || '')
            : 'memo@';
        const thinoInsertHeading = thinoCompatible ? (this.config["thinoInsertHeading"] || '') : '';
        ...
        const memoDir = thinoCompatible
            ? (0, obsidian_1.normalizePath)(flomoTarget)
            : (0, obsidian_1.normalizePath)(`${flomoTarget}/${memoTarget}`);
        ...
        for (const date in memosByDate) {
            ...
            if (thinoCompatible) {
                ...
                const filePath = (0, obsidian_1.normalizePath)(`${memoDir}/${fileName}`);
                ...
                // ===== 原始模式：三层结构 flomoTarget/memoTarget/日期/文件 =====
                const dateDir = (0, obsidian_1.normalizePath)(`${memoDir}/${date}`);
                await this.app.vault.adapter.mkdir(dateDir);
                if (mergeByDate) {
                    // 按日期合并：一天一个文件 memo@2025-01-12.md
                    const fileName = `${filenamePrefix}${date}.md`;
                    const filePath = (0, obsidian_1.normalizePath)(`${dateDir}/${fileName}`);
                    const newContent = dateData.contents.join(separator);
                    const fileExists = await this.app.vault.adapter.exists(filePath);
                    if (fileExists) {
                        const existingContent = await this.app.vault.adapter.read(filePath);
    ...
    // 在指定标题下插入内容
    async insertUnderHeading(filePath, heading, newContent, separator) {
        const existingContent = await this.app.vault.adapter.read(filePath);
    ...
    async copyAttachmentsSkipUserIdDir(sourceDir, targetDir) {
        try {
            ...
            for (const dateItem of dateItems) {
                ...
                const targetDateDir = (0, obsidian_1.normalizePath)(`${targetDir}/${dateItem.name}`);
                // 递归收集所有文件（扁平化）
                const filesToCopy = [];
                // 递归函数：收集目录下所有文件
                const collectFiles = async (dir) => {
                    ...
                    for (const item of items) {
                        ...
                        if (item.isFile()) {
                            ...
                            filesToCopy.push({
                                source: itemPath,
                                target: (0, obsidian_1.normalizePath)(`${targetDateDir}/${item.name}`)
                            });
                        }
    ...
    async import() {
        // 1. Create workspace
        const tmpDir = path ? path.join(const_1.FLOMO_CACHE_LOC, "data") : `${const_1.FLOMO_CACHE_LOC}/data`;
        ...
        const flomoTarget = this.config["flomoTarget"] || "flomo";
        let attachementDir = (0, obsidian_1.normalizePath)(`${flomoTarget}/flomo attachment`);
        ...
        const flomo = new core_1.FlomoCore(dataExport, syncedMemoIdsForCore, flomoTarget, filterTags, thinoCompatible, useFirstLineDedup);
    ...
    async importFlomoFile(filePath, mergeDayFile = true) {
        ...
        const flomo = new core_1.FlomoCore(flomoData, syncedMemoIds, flomoTarget, filterTags, thinoCompatible);
