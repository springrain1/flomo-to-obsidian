...
export class FlomoCore {
    ...
    filterTags: string[] = []; // 要筛选的标签
    thinoCompatible: boolean = false; // Thino 兼容模式
    skipAutoAddSyncedIds: boolean = false; // 是否跳过自动添加 syncedMemoIds（首行去重模式使用）

    constructor(flomoData: string, syncedMemoIds: string[] = [], flomoTarget: string = 'flomo', filterTags: string[] = [], thinoCompatible: boolean = false, skipAutoAddSyncedIds: boolean = false) {
        ...
        memoNodes.forEach(i => {
            ...
            if (this.filterTags.length > 0) {
                ...
                const hasMatchingTag = this.filterTags.some(filterTag => {
                    ...
                    return memoTagsNormalized.some(memoTag =>
                        memoTag === normalizedFilter || memoTag.startsWith(normalizedFilter + '/')
                    );
                });
